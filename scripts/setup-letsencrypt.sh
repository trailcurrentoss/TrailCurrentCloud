...
display_next_steps() {
    ...
    echo "   https://$DOMAIN"
    echo ""
    echo "4. Set up automatic certificate renewal (cron):"
    echo "   crontab -e"
    echo "   0 0,12 * * * $SCRIPT_DIR/renew-certs.sh >> $PROJECT_ROOT/logs/cert-renewal.log 2>&1"
    echo ""
    echo "   Create the logs directory: mkdir -p $PROJECT_ROOT/logs"
    echo ""
